...
(private var location: Location?, private var method: Method?) {
     ...
     * prayers
     */
    fun getPrayerTimes(date: LocalDateTime): AzanTimes {
     ...
     */
    fun getPrayerTimes(date: LocalDateTime, pt: AzanTimes) {
    ...
    fun ieeeRemainder(dividend: Double, divisor: Double): Double {
        ...
        val adjustedRemainder = abs(remainder) * sign(dividend)
        return if (abs(remainder) == abs(adjustedRemainder)) {
            adjustedRemainder
        } else {
            val halfDivisor = divisor / 2.0
     ...
     * @return imsaak time
     */
    fun getImsaak(date: LocalDateTime): Time {
     ...
     */
    fun getNextDayImsaak(date: LocalDateTime): Time {
     ...
     */
    fun getNextDayFajr(date: LocalDateTime): Time {
    ...
    companion object {
        ...
        internal fun getFajIsh(Lat: Double, dec: Double, Ang: Double): Double {
            ...
            return if (part3 <= Utils.INVALID_TRIGGER) {
                99.0
            } else Utils.DEG_TO_10_BASE * Utils.RAD_TO_DEG(   acos(part3))
        ...
        internal fun getShoMag(loc: Location, astro: Astro, type: PrayerTime): Double {
            ...
            lhour = AstrologyFormulas.limitAngle180(Utils.RAD_TO_DEG(   acos(part4)))
            ...
            R = M + (sunAlt - Utils.CENTER_OF_SUN_ANGLE + Utils.ALTITUDE_REFRACTION *  loc.seaLevel.pow( 0.5)) / (360.0
                    *    cos(Utils.DEG_TO_RAD(B))
        ...
        internal fun getAssr(Lat: Double, dec: Double, madhhab: Madhhab?): Double {
            ...
            return Utils.DEG_TO_10_BASE * Utils.RAD_TO_DEG(   acos(part4))
        }
        ...
        /* Obtaining the direction of the shortest distance towards Qibla angle by u   sing the
	 * great circle formula */
        ...
        fun getAdjustmentTimeToQiblaDirection(loc: Location): SimpleTime {
            ...
            num =    sin(Utils.DEG_TO_RAD(loc.degreeLong) - Utils.DEG_TO_RAD(Utils.KAABA_LONG))
            denom =    cos(Utils.DEG_TO_RAD(loc.degreeLat)) * tan(Utils.DEG_TO_RAD(Utils.KAABA_LAT)) -    sin(Utils.DEG_TO_RAD(loc.degreeLat)) * cos(Utils.DEG_TO_RAD(loc.degreeLong) - Utils
