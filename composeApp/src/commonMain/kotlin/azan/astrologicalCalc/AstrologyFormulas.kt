...
object AstrologyFormulas {
    ...
    fun getRefraction(loc: Location, sunAlt: Double): Double {
        ...
        part2 = 1.02 / (Utils.RAD_TO_DEG(   tan(Utils.DEG_TO_RAD(sunAlt + 10.3 / (sunAlt + 5.11)))) + 0.0019279)
    ...
    fun getJulianDay(date: SimpleDate, gmt: Double): Double {
        ...
        JD = (   floor(365.25 * (jdY + 4716.0))
                +    floor(30.6001 * (jdM + 1)) + (date.day + -gmt / 24.0)
                + jdB) - 1524.5
    ...
    fun computeAstroDay(JD: Double, astroday: AstrologyDay) {
        ...
        tL = (L0sum + L1sum * JM + L2sum * JM2 + L3sum * JM3
                + L4sum * JM4 + L5sum * JM5) /    pow(10.0, 8.0)
        ...
        tB = (B0sum + B1sum * JM) /    pow(10.0, 8.0)
        ...
        R4sum = R4[0] *    cos(R4[1] + R4[2] * JM)

        R = (R0sum + R1sum * JM + R2sum * JM2 + R3sum * JM3 + R4sum * JM4) /    pow(10.0, 8.0)

        G = limitAngle(L + 180)
        Gg = -B

        X0 = 297.85036 + 445267.111480 * JC - 0.0019142 *    pow(JC, 2.0) +    pow(JC, 3.0) / 189474.0
        X1 = (357.52772 + 35999.050340 * JC - 0.0001603 *    pow(JC, 2.0)
                -    pow(JC, 3.0) / 300000.0)
        X2 = (134.96298 + 477198.867398 * JC + 0.0086972 *    pow(JC, 2.0)
                +    pow(JC, 3.0) / 56250.0)
        X3 = 93.27191 + 483202.017538 * JC - 0.0036825 *    pow(JC, 2.0) +    pow(JC, 3.0) / 327270.0
        X4 = (125.04452 - 1934.136261 * JC + 0.0020708 *    pow(JC, 2.0)
                +    pow(JC, 3.0) / 450000.0)
        ...
        while (i < 63) {
            ...
            psi += (PE[i][0] + JC * PE[i][1]) *    sin(Utils.DEG_TO_RAD(xsum))
            epsilon += (PE[i][2] + JC * PE[i][3]) *    cos(Utils.DEG_TO_RAD(xsum))
            xsum = 0.0
            i++
        }
        ...
        U = JM / 10.0
        E0 = 84381.448 - 4680.93 * U - 1.55 *    pow(U, 2.0) + 1999.25 *    pow(U, 3.0) - 51.38 *    pow(U, 4.0) - 249.67 *    pow(U, 5.0) - 39.05 *    pow(U, 6.0) + 7.12 *    pow(U, 7.0) + 27.87 *    pow(U, 8.0) + 5.79 *    pow(U, 9.0) + 2.45 *    pow(U, 10.0)
        E = E0 / 3600.0 + deltaEps
        lamda = G + deltaPsi + -20.4898 / (3600.0 * R)

        V0 = 280.46061837 + 360.98564736629 * (JD - 2451545) + 0.000387933 *    pow(JC, 2.0) -    pow(JC, 3.0) / 38710000.0
        V = limitAngle(V0) + deltaPsi *    cos(Utils.DEG_TO_RAD(E))

        RAn =    sin(Utils.DEG_TO_RAD(lamda)) *    cos(Utils.DEG_TO_RAD(E)) -    tan(Utils.DEG_TO_RAD(Gg)) *    sin(Utils.DEG_TO_RAD(E))
        RAd =    cos(Utils.DEG_TO_RAD(lamda))
        RA = limitAngle(Utils.RAD_TO_DEG(   atan2(RAn, RAd)))

        DEC =    asin(   sin(Utils.DEG_TO_RAD(Gg)) *    cos(Utils.DEG_TO_RAD(E)) + (   cos(Utils.DEG_TO_RAD(Gg))
                *    sin(Utils.DEG_TO_RAD(E))
                *    sin(Utils.DEG_TO_RAD(lamda))))
    ...
    fun computeTopAstro(loc: Location, astro: Astro,
        ...
        while (i < 3) {
            ...
            tU =    atan(0.99664719 *    tan(Utils.DEG_TO_RAD(loc
                    .degreeLat)))

            tCos =    cos(tU) + loc.seaLevel / 6378140.0 *    cos(Utils.DEG_TO_RAD(loc.degreeLat))

            tSin = 0.99664719 *    sin(tU) + loc.seaLevel / 6378140.0 *    sin(Utils.DEG_TO_RAD(loc.degreeLat))

            tRA0 = -tCos *    sin(Utils.DEG_TO_RAD(SP)) *    sin(Utils
                    .DEG_TO_RAD(lHour)) / (   cos(astro.dec[i]) - (tCos
                    *    sin(Utils.DEG_TO_RAD(SP))
                    *    cos(Utils.DEG_TO_RAD(lHour))))

            tRA = astro.ra[i] + Utils.RAD_TO_DEG(tRA0)

            tDEC = Utils.RAD_TO_DEG(   atan2(
                    (   sin(astro.dec[i]) - tSin *    sin(Utils.DEG_TO_RAD(SP))) *    cos(tRA0),    cos(astro.dec[i]) - (tCos *    sin(Utils.DEG_TO_RAD(SP))
                    *    cos(Utils.DEG_TO_RAD(lHour)))))
    ...
    fun limitAngle180between(L: Double): Double {
        ...
        limit /= 360.0
        limitFactor = (limit -    floor(limit)) * 360.0
        if (limitFactor < -180)
            limitFactor += 360.0
